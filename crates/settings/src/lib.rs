...
impl Settings {
    ...
    fn to_toml_string(&self) -> String {
        toml::to_string_pretty(self).unwrap_or_else(|err| {
...
mod tests {
    ...
    #[test]
    fn app_settings_toml_writer_round_trips_theme_and_cursor_values() {
        let mut settings = Settings::default();
        settings.theme_family =
            crate::categories::appearance::AppearanceSettings::default().theme_family;
        settings.theme_variant = theme::ThemeVariant::RosePineMoon;
        settings.cursor_style = CursorStyle::Underline;
        settings.search_ignored_dirs = "foo,bar".to_string();

        let serialized = settings.to_toml_string();
        let deserialized: Settings = toml::from_str(&serialized).unwrap();

        assert_eq!(deserialized.theme_family, settings.theme_family);
        assert_eq!(deserialized.theme_variant, settings.theme_variant);
        assert_eq!(deserialized.cursor_style, settings.cursor_style);
    }
}
...
impl Settings {
                ...
                |ui| {
                    ...
                    egui::ScrollArea::both()
                        ...
                        .show(ui, |ui| match self.selected_category {
                            SettingsCategory::About => self.show_about_settings(ui),
                            SettingsCategory::Appearance => self.show_appearance_settings(
                                ui,
                                has_search,
                                &self.search_query.clone(),
                            ),
                            SettingsCategory::Editor => self.show_editor_settings(
                                ui,
                                has_search,
                                &self.search_query.clone(),
                            ),
                            ...
                            SettingsCategory::Keyboard => self.show_keyboard_settings(
                                ui,
                                has_search,
                                &self.search_query.clone(),
                            ),
                            SettingsCategory::Search => self.show_search_settings(
                                ui,
                                has_search,
                                &self.search_query.clone(),
                            ),
                            SettingsCategory::Workbench => self.show_workbench_settings(
                                ui,
                                has_search,
                                &self.search_query.clone(),
                            ),
    ...
    fn count_matching_settings(&self, query: &str) -> usize {
        ...
        let search_settings = [
            "search ignore directories enabled",
            "search ignored directories",
            "search min chars",
            "search case sensitive",
            "search whole word",
            "search use regex",
            "search include hidden",
    ...
    fn show_keyboard_settings(&mut self, ui: &mut egui::Ui, has_search: bool, query: &str) {
            ...
            self.setting_card(ui, "Vim", |ui, settings| {
                ...
                if settings.vim_mode {
                    settings.cozy_row_filtered(
                        ...
                        |ui, settings| {
                            ui.add(
                                egui::TextEdit::singleline(&mut settings.vim_leader_key)
                                    .desired_width(40.0)
        ...
        if !has_search
            || self.matches_search(
                &query,
                &[
                    "editor", "features", "multi", "cursor", "bracket", "snippet",
                ],
            )
        {
    ...
    fn show_editor_settings(&mut self, ui: &mut egui::Ui, has_search: bool, query: &str) {
        ...
        if !has_search || self.matches_search(&query, &["font", "text", "editor"]) {
            self.setting_card(ui, "Font", |ui, settings| {
                settings.cozy_row_filtered(
                    ...
                    |ui, settings| {
                        ui.add(
                            egui::TextEdit::singleline(&mut settings.font_family)
                                .hint_text("SF Mono")
        ...
        if !has_search
            || self.matches_search(&query, &["behavior", "vim", "auto", "save", "scroll"])
        {
    ...
    fn show_file_settings(&mut self, ui: &mut egui::Ui, has_search: bool, query: &str) {
        ...
        if !has_search
            || self.matches_search(&query, &["file", "handling", "auto", "save", "whitespace"])
        {
        ...
        if !has_search || self.matches_search(&query, &["encoding", "charset", "utf"]) {
            self.setting_card(ui, "Encoding", |ui, settings| {
                settings.cozy_row_filtered(
                    ...
                    |ui, settings| {
                        ui.add(
                            egui::TextEdit::singleline(&mut settings.default_file_encoding)
                                .hint_text("utf8")
        ...
        if !has_search
            || self.matches_search(&query, &["association", "extension", "language", "mapping"])
        {
            self.setting_card(ui, "File Associations", |ui, settings| {
                ui.add(
                    egui::Label::new(egui::RichText::new("Add Association").size(13.0).strong())
                        .selectable(false),
                );
                ui.add(
                    egui::Label::new(
                        egui::RichText::new("Map file extension to language mode")
                            .size(11.0)
                            .color(theme::CherryBlossomTheme::text_muted()),
                    )
                    .selectable(false),
                ...
                egui::Grid::new("file_assoc_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.add(
                            egui::TextEdit::singleline(&mut settings.new_file_ext_input)
                                .hint_text(".rs")
                        ...
                        ui.horizontal(|ui| {
                            ui.add(
                                egui::TextEdit::singleline(&mut settings.new_file_lang_input)
                                    .hint_text("rust")
                            ...
                            if ui.button("Add").clicked()
                                && !settings.new_file_ext_input.is_empty()
                                && !settings.new_file_lang_input.is_empty()
                            {
                                settings.file_associations.insert(
                                    settings.new_file_ext_input.clone(),
                                    settings.new_file_lang_input.clone(),
                                );
                ...
                if !settings.file_associations.is_empty() {
                    ui.add(
                        egui::Label::new(
                            egui::RichText::new("Existing Associations")
                                .size(13.0)
                                .strong(),
                        )
                        .selectable(false),
                    ...
                    egui::Grid::new("existing_assocs_grid")
                        .num_columns(2)
                        .spacing([16.0, 4.0])
                        .show(ui, |ui| {
                            let associations: Vec<_> =
                                settings.file_associations.clone().into_iter().collect();
                            for (ext, lang) in associations.into_iter() {
                                ui.add(
                                    egui::Label::new(format!("{} : {}", ext, lang))
                                        .selectable(false),
                                );
