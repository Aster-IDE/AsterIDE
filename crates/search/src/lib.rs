...
pub fn show_search_tab(ui: &mut egui::Ui, state: &mut SearchState, min_chars: usize) {
  ui.vertical(|ui| {
    ...
    ui.horizontal(|ui| {
      ...
      if ui
        .add(
          egui::Label::new(egui::RichText::new("Aa").color(case_color).strong())
      ...
      if ui
        .add(
          egui::Label::new(egui::RichText::new("\\b").color(word_color).monospace())
            .sense(egui::Sense::click()),
        )
        .clicked()
      {
      ...
      if ui
        .add(
          egui::Label::new(egui::RichText::new(".*").color(regex_color).monospace())
            .sense(egui::Sense::click()),
        )
        .clicked()
      {
          ...
          egui::Frame::new()
            ...
            .show(ui, |ui| {
              ui.horizontal(|ui| {
                ...
                let matched = &result.line_content[result.start_col.saturating_sub(1)
                  ..result
                    .end_col
                    .saturating_sub(1)
                    .min(result.line_content.len())];
                let after = &result.line_content[result
                  .end_col
                  .saturating_sub(1)
                  .min(result.line_content.len())..];
